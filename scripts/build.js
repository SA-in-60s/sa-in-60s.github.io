    ...
    <article>
      ...
      <div class="md:flex md:gap-8 mb-8">
        ...
        <div class="md:w-2/3">
          ...
          <div data-lang="de" class="mt-4">
            <a href="https://claude.ai/new?q=${encodeURIComponent(`Ich habe gerade ein 60-Sekunden-Video über "${concept.title_de}" auf sa-in-60s.github.io gesehen.\n\nErkläre mir das Konzept tiefer:\n- Was sind typische Praxisbeispiele?\n- Welche Vor- und Nachteile gibt es?\n- Wie hängt es mit verwandten Konzepten zusammen?\n- Wann sollte man es einsetzen — und wann nicht?`)}" target="_blank" rel="noopener" class="inline-flex items-center gap-2 py-2 px-4 rounded-lg border border-text-muted text-text-muted hover:text-accent-cyan hover:border-accent-cyan transition text-sm">🤖 Mit Claude tiefer einsteigen</a>
          </div>
          <div data-lang="en" class="hidden mt-4">
            <a href="https://claude.ai/new?q=${encodeURIComponent(`I just watched a 60-second video about "${concept.title_en}" on sa-in-60s.github.io.\n\nExplain this concept in more depth:\n- What are typical real-world examples?\n- What are the pros and cons?\n- How does it relate to other concepts?\n- When should you use it — and when not?`)}" target="_blank" rel="noopener" class="inline-flex items-center gap-2 py-2 px-4 rounded-lg border border-text-muted text-text-muted hover:text-accent-cyan hover:border-accent-cyan transition text-sm">🤖 Explore deeper with Claude</a>
          </div>
        </div>
      </div>
